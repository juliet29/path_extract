...
def get_emissions_df(df: pl.DataFrame):
    d = df.filter(pl.col(ClassNames.VALUE.name) > 0).filter(
        ...
        df.with_columns(
            (
                pl.coalesce(
                    ...
                    pl.when(pl.col(ClassNames.CATEGORY.name) == cond).then(
                        pl.lit(result.name)
                    )
...
def compare_two_experiments(baseline: pl.DataFrame, alternative: pl.DataFrame):
    ...
    df = baseline.join(
        ...
        how="right",
        suffix="_ALT",
    ).with_columns(pl.col(Columns.VALUE.name).fill_null(strategy="zero")).rename({Columns.VALUE.name: Columns.BASELINE.name, Columns.VALUE_ALT.name: Columns.ALT.name})
    ...
    # with pl.Config(tbl_rows=-1):
    #     rprint(
    #         df2.select([pl.col(Columns.ELEMENT.name), pl.col(Columns.VALUE_DIFF.name)]).sort(by=Columns.ELEMENT.name)
    #     )
