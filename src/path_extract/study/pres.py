...
from path_extract.study.plots.constants import RendererTypes
from path_extract.study.plots.plots import plot_elements, plot_use_categories
import altair as alt
from path_extract.study.plots.comparison import get_net_emissions
from path_extract.study.plots.waterfall import prep_dataframe, make_waterfall_chart
...
def make_waterfall_figure(
    ...
    if renderer == HTML:
        chart.save(
            clmt_path.figures_path / f"exp{base_exp_num}_{alt_exp_num}_waterfall.png",
            format="png",
            ppi=200,
        )
    else:
...
def make_categ_figure(
    ...
    clmt_path = CLMTPath(project_name)
    init_df = clmt_path.read_csv(exp_num)  # TODO change to read or get
...
def make_project_figures():
    # TODO: would be nice to set up constants with the scenarios to be studied..
    # pier 6
...
def get_project_comparisons(
    ...
    rprint(
        f"Project: {project_name} | Percentage Increase: {round(p_inc)} | Base Emit ({base_exp_num}): {round(base_emit)} | Alt Emit ({round(alt_emit)})\n"
    )


if __name__ == "__main__":
    # pier 6
    get_project_comparisons(
        "pier_6",
        0,
        1,
    )
